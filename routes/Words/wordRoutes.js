...
router.get('/single-word/:wordId', (req, res) => {
  ...
  .then((dbWord) => {
    if(dbWord) {
      ...
      return res.status(400).send("No Words FOund");
    }
  })
  .catch((err) => {
    return res
...
return res.status(400).send('No Word Found');
    ...
    return res.status(200).render('main/update-word', { foundWord: foundWord}) //the dbword
...
router.put('/update/:wordId', (req, res) => {
  Word.findById(req.params.wordId).then((foundWord) => {
    if (!foundWord) {
      return res.status(400).send('No Word Found');
...
router.delete("/delete/:wordId", (req, res) => {
  Word.findByIdAndDelete(req.params.wordId).then((foundWord) => {
    if(!foundWord) {
